...
the known-tools registry (precommit-tools.yaml), and outputs a JSON
manifest to stdout.
    ...
    for repo_entry in repos:
        ...
        for hook in repo_entry.get("hooks", []):
            ...
            if tool is None and repo_url == "local":
                entry_cmd = entry.split()[0] if entry else ""
...
def main():
    ...
    registry = os.path.join(script_dir, "precommit-tools.yaml")
