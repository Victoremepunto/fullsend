...
func TestMintAgentToken_SkipsWhenNoMintURL(t *testing.T) {
	printer := ui.New(io.Discard)
	minted, err := mintAgentToken(context.Background(), "coder", "", printer)
	require.NoError(t, err)
...
func TestMintAgentToken_SkipsWhenNoRole(t *testing.T) {
	printer := ui.New(io.Discard)
	minted, err := mintAgentToken(context.Background(), "", "https://mint.example.com", printer)
	require.NoError(t, err)
	assert.False(t, minted)
}
...
func TestMintAgentToken_CoderRole(t *testing.T) {
	...
	var buf bytes.Buffer
	printer := ui.New(&buf)
	minted, err := mintAgentToken(context.Background(), "coder", "https://mint.example.com", printer)
	require.NoError(t, err)
...
func TestMintAgentToken_ReviewRole(t *testing.T) {
	...
	printer := ui.New(io.Discard)
	minted, err := mintAgentToken(context.Background(), "review", "https://mint.example.com", printer)
	require.NoError(t, err)
	assert.True(t, minted)
...
func TestMintAgentToken_RetroRole(t *testing.T) {
	origMint := statusMintToken
	defer func() { statusMintToken = origMint }()
	...
	t.Setenv("GH_TOKEN", "")
	t.Setenv("RETRO_SANDBOX_TOKEN", "")

	printer := ui.New(io.Discard)
	minted, err := mintAgentToken(context.Background(), "retro", "https://mint.example.com", printer)
	require.NoError(t, err)
	assert.True(t, minted)

	assert.Equal(t, "ghs_retro_token", os.Getenv("GH_TOKEN"))
	assert.Equal(t, "ghs_retro_token", os.Getenv("RETRO_SANDBOX_TOKEN"))
}
...
func TestMintAgentToken_ResolvesAliases(t *testing.T) {
	...
	printer := ui.New(io.Discard)
	minted, err := mintAgentToken(context.Background(), "code", "https://mint.example.com", printer)
	require.NoError(t, err)
	assert.True(t, minted)
...
func TestMintAgentToken_TriageRole_NoExtras(t *testing.T) {
	...
	printer := ui.New(io.Discard)
	minted, err := mintAgentToken(context.Background(), "triage", "https://mint.example.com", printer)
	require.NoError(t, err)
	assert.True(t, minted)
...
func TestMintAgentToken_MintError(t *testing.T) {
	...
	printer := ui.New(io.Discard)
	_, err := mintAgentToken(context.Background(), "coder", "https://mint.example.com", printer)
	require.Error(t, err)
...
func TestMintAgentToken_RejectsInvalidRole(t *testing.T) {
	...
	printer := ui.New(io.Discard)
	_, err := mintAgentToken(context.Background(), "INVALID--ROLE", "https://mint.example.com", printer)
	require.Error(t, err)
...
func TestRoleTokenVars_Coverage(t *testing.T) {
	assert.Equal(t, []string{"PUSH_TOKEN", "PUSH_TOKEN_SOURCE"}, roleTokenVars["coder"])
	assert.Equal(t, []string{"REVIEW_TOKEN"}, roleTokenVars["review"])
	assert.Equal(t, []string{"RETRO_SANDBOX_TOKEN"}, roleTokenVars["retro"])
